...
base.tar:
	...
	docker run --name manjarowsl manjarolinux/base:latest /bin/bash -c "pacman-mirrors --fasttrack 5; pacman --noconfirm -Sy awk; pacman-key --init; pacman-key --populate archlinux; sed -ibak -e 's/#Color/Color/g' -e 's/CheckSpace/#CheckSpace/g' /etc/pacman.conf; pacman --noconfirm -Syyu; pacman-mirrors --api --set-branch testing; pacman-mirrors --fasttrack 5; pacman --noconfirm -Syyuu; pacman --noconfirm -R dbus; pacman --noconfirm --needed -S aria2 aspell base-devel ccache dbus-x11 dconf git grep hspell hunspell inetutils iputils keychain libvoikko nano nuspell openssh procps sudo wget; setcap 'cap_net_admin,cap_net_raw+ep' /usr/sbin/ping; echo '%wheel ALL=(ALL) ALL' > /etc/sudoers.d/wheel; ip link set dev eth0 mtu 1500; rm /var/lib/dbus/machine-id; dbus-uuidgen --ensure=/etc/machine-id; dbus-uuidgen --ensure; yes | LC_ALL=en_US.UTF-8 pacman -Scc"
