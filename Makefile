...
base.tar:
	...
	docker run --net=host --name manjarowsl manjarolinux/base:latest /bin/bash -c "pacman-mirrors -c Global; pacman-key --init; pacman-key --populate; sed -ibak -e 's/#Color/Color/g' -e 's/CheckSpace/#CheckSpace/g' /etc/pacman.conf; sed -ibak -e 's/IgnorePkg/#IgnorePkg/g' /etc/pacman.conf; pacman --noconfirm -Syyu; pacman --noconfirm -Rdd dbus; pacman --noconfirm --needed -Sy aria2 aspell base-devel ccache dbus-x11 dconf figlet git grep hspell hunspell hwdata inetutils iputils iproute2 keychain libvoikko linux-tools lolcat nano ntp nuspell openssh procps sudo usbutils vi vim wget; mkdir -p /etc/pacman.d/hooks; echo '%wheel ALL=(ALL) ALL' > /etc/sudoers.d/wheel; yes | LC_ALL=en_US.UTF-8 pacman -Scc"
