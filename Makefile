...
base.tar:
	...
	docker run --name manjarowsl manjarolinux/base:latest /bin/bash -c "pacman --noconfirm -Sy awk; pacman-key --init; pacman-key --populate archlinux; pacman --noconfirm -Syyu; pacman-mirrors --api --set-branch testing; pacman-mirrors --fasttrack 5; pacman --noconfirm -Syyuu; pacman --noconfirm -S aria2 base-devel ccache git git-lfs grep inetutils iputils linux-tools lzip nano openssh procps sudo tree vivid wget; setcap 'cap_net_admin,cap_net_raw+ep' /usr/sbin/ping; yes | LC_ALL=en_US.UTF-8 pacman -Scc"
