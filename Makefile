...
base.tar:
	...
	docker run --name manjarowsl manjarolinux/base:latest /bin/bash -c "pacman --noconfirm -Sy awk; pacman-key --init; pacman-key --populate archlinux; sed -ibak -e 's/#Color/Color/g' -e 's/CheckSpace/#CheckSpace/g' /etc/pacman.conf; pacman --noconfirm -Syyu; pacman-mirrors --api --set-branch testing; pacman-mirrors --fasttrack 5; pacman --noconfirm -Syyuu; pacman --noconfirm --needed -S aria2 base-devel ccache git git-lfs grep inetutils iputils linux-tools lsop lzip nano openssh procps socat sudo tree vivid wget; setcap 'cap_net_admin,cap_net_raw+ep' /usr/sbin/ping; sed -i 's/# %wheel ALL=(ALL) ALL/%wheel ALL=(ALL) ALL/g' /etc/sudoers; yes | LC_ALL=en_US.UTF-8 pacman -Scc"
